...
void Send_Framebuffer_To_Display(void)
  ...
  for(page = 0; page < 8; page++)
  {
    ...
    for(column=0;column<128;column++)
    {
      ...
      wiringPiSPIDataRW(CHANNEL, source, 1);
      source++;
    }
  }
#endif
    ...
    for (column = 0; column < 128 / 8; column++)
    {
      for (i = 0; i < 8; i++)
      {
        ...
        for (j = 0; j < 8; j++)
        {
          ...
          // Read the byte
          theByte = NFH_Mono_1BPP[coordinate];
...
void put_string(uint8_t x, uint8_t y, uint8_t Field_Width, const char *input)
  {
  static uint8_t Terminator_Found;
  static uint8_t Characters_Placed;
  static uint8_t this_character;
  static uint8_t *LCD_Memory;
  static uint8_t column;
  static uint8_t row;
  WORD_UNION Clearing_Mask;
...
void invert_string(uint8_t x, uint8_t y, uint8_t Field_Width)
{
  ...
  static uint8_t column;
  static uint8_t row;
  static uint8_t Columns_To_Invert;
  WORD_UNION Inverting_Mask;
...
void clear_rectangle(uint8_t x1,uint8_t y1,uint8_t x2,uint8_t y2)
  {
  static uint8_t *LCD_Memory;
  static uint8_t *LCD_Memory_1;
  static uint8_t *LCD_Memory_2;
  static uint8_t mask;
  static uint8_t column;
